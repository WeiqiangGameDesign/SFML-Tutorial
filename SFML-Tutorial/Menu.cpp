...
Menu::Menu(float width, float height)
{
	...
	menu[0].setFont(font);
	menu[0].setString("1. Player With Computer");
	menu[0].setFillColor(sf::Color::Red);
	menu[0].setPosition(sf::Vector2f(width / 2 - 150.f, (height / (MAX_NUM_OF_ITEMS + 1)) * 2 - 50.f));


	menu[1].setFont(font);
	menu[1].setString("2. Play With Human");
	menu[1].setFillColor(sf::Color::White);
	menu[1].setPosition(sf::Vector2f(width / 2 - 150.f, (height / (MAX_NUM_OF_ITEMS + 1)) * 2));
